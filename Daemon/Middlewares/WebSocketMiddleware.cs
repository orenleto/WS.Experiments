...
public class WebSocketMiddleware : IMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, RequestDelegate next)
    {
        try
        {
            if (SocketLoopTokenSource.IsCancellationRequested)
            {
    ...
    private static async Task CloseAllSocketsAsync()
    {
        ...
        while (_clients.Count > 0)
        {
            ...
            {
                var timeout = new CancellationTokenSource(TimeSpan.FromMilliseconds(3000));
                try
                {
...
internal record SubscribeChangesCommand(string Path);
