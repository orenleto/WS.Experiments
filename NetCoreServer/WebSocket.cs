...
public class WebSocket : IWebSocket
{
    ...
    {
        var storeWSCloseStatus = (opcode & WS_CLOSE) == WS_CLOSE && buffer.Length > 0;
        ...
        lock (WsReceiveLock)
        {
            ...
            while (size > 0)
            {
                ...
                else if (payload == 127)
                {
                    ...
                    payload = (WsReceiveFrameBuffer[2] << 56) | (WsReceiveFrameBuffer[3] << 48) | (WsReceiveFrameBuffer[4] << 40) |
                              (WsReceiveFrameBuffer[5] << 32) | (WsReceiveFrameBuffer[6] << 24) | (WsReceiveFrameBuffer[7] << 16) |
